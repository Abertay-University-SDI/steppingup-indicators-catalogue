...
const C = {
	...
	EC_COLOUR_HIGHLIGHT: "hsl(200, 100%, 50%)",
	ENV_COLOUR_HIGHLIGHT: "hsl(100, 100%, 45%)",
	SOC_COLOUR_HIGHLIGHT: "hsl(40, 100%, 50%)",
	MIX_COLOUR_HIGHLIGHT: "hsl(260, 100%, 65%)",
		...
		.on("mouseover", function(d, i) {
			...
			d3.selectAll("path.tablet").each(function(dd, ii) {
				if (dd[d] === 1) {
					...
					.attr("fill", function() {
						switch (dd.type) {
							case "Ec": return C.EC_COLOUR_HIGHLIGHT; break;
							case "Soc": return C.SOC_COLOUR_HIGHLIGHT; break;
							case "Env": return C.ENV_COLOUR_HIGHLIGHT; break;
							default: return C.MIX_COLOUR_HIGHLIGHT; break;
						}
				});
				}
			});
		...
		.on("mouseover", function(d, i) {
			if (!d3.select(this).classed("opened")) {
				...
				if (!d3.select(this).classed("selected")) {
					...
					createLabel(d, i, C.IN_RADIUS + C.IN_RADIUS_PUSH - 15);
					d3.select("#lab" + String(i)).style("font-weight", "bold");
				} else {
					selectIndicator(this, d, i, C.IN_RADIUS + C.IN_RADIUS_PUSH - 15, C.OUT_RADIUS + C.OUT_RADIUS_PUSH + 30);
					d3.select("#lab" + String(i)).style("font-weight", "bold");
